...
렌더링 방식의 속도를 비교할 때 웹 바이탈 지표로 보면 더욱 명확해집니다.
...
| 지표 | 시간 | 설명 |
|---|---|---|
| TTFB | 570ms | 빈 HTML를 반환하며 빠릅니다. |
| FCP | 1.6s | HTML 수신 → JS 요청 → JS 실행 → DOM 조작 순서를 거쳐야 첫 콘텐츠가 표시됩니다. JS 번들이 커질 수록 길어질 수 있습니다. |
| LCP | 5.9s | 워터폴로 API를 요청할 경우 더 길어질 수 있습니다. |

TTFB가 빠르고, FCP와 LCP는 처리에 따라 더 느려질 수 있습니다.
...
| 지표 | 시간 | 설명 |
|---|---|---|
| TTFB | 3.29s | 정적 SSR에선 빠르지만, 동적 SSR에서 데이터 fetch 등 작업이 많을수록 느려질 수 있습니다. |
| FCP | 1.5s | 브라우저가 HTML을 받으면 바로 콘텐츠를 표시할 수 있어 빠릅니다. |
| LCP | 1.9s | 주요 콘텐츠가 이미 HTML에 포함되어 있다면 FCP와 거의 비슷하게 빠릅니다. |
